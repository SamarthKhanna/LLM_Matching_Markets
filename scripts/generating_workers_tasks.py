...
model_folder = f"../evaluating_responses/part_1c/{model_type}/"
...
for instance_file in instance_files:
    ...
    result_file = f'../evaluating_responses/part_1c/{model_type}_{culture}_{size}_repeat.csv'
    if os.path.exists(result_file):
        print(f"File \"{result_file}\" already exists!")
        continue
    intermediate_folder = f'../evaluating_responses/part_1c/{model_type}/{culture}_{size}/'
    ...
    pg.get_prompts_list(task_worker=True)
    ...
    for p, prompt in enumerate(tqdm(prompts_list)):
        ...
        for r in range(2):
            ...
            try:
                ...
                prompt_req = pg.correct_json_prompt(og_prompt, response, size, task_worker=True)
                # print(prompt_req)
                continue
            ...
            if verdict not in ['empty', 'okay']:
                ...
                print(answer_ext)
                print(answer)
                prompt_req = pg.correct_json_obj_prompt(og_prompt, response, verdict, size, task_worker=True)
                # print(prompt_req)
                continue
            ...
            if len(set(llm_answer_list)) < size or 0 in llm_answer_list:
                ...
                for m, woman in enumerate(llm_answer_list):
                    ...
                    if woman != 0 and len(counts[woman]) > 1:
                        invalid = True
                prompt_req = pg.incomplete_matching_prompt(og_prompt, response, counts, set([w+1 for w in range(size)]) - set(llm_answer_list),size, task_worker=True)
                if invalid:
